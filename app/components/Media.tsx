...
const media: Media[] = [
  ...
  {
    title: "No One Can Refuse Doggy A",
    finishedDate: "Fall 2024",
        ...
        {media
          ...
          .map((item) => (
            <div className="py-4">
                ...
                {item.tags
                  .sort((a, b) => {
                    for (const category of tagCategories) {
                      ...
                      // If both tags are not in the category, continue
                      if (indexA === -1 && indexB === -1) {
                        continue;
                      }
                      // If one tag is not in the category, put it at the end
                      if (indexA === -1 || indexB === -1) {
                        return indexB - indexA;
                      }
                      // Otherwise, sort by the index in the category
              ...
              {item.comments && (
                <div className="text-gray-600 text-sm py-1 whitespace-pre-line">
                  {item.comments}
                </div>
              )}
            </div>
