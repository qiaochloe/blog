...
7. [Toolchain-Based Hardening](#toolchain-based-hardening)
...
1. Get the return address through a memory disclosure vulnerability.

2. Calculate the base address of the main executable by subtracting the offset of the return address from the base address. 
...
3. Calculate the addresses of gadgets and functions in the main executable 
    ...
    Note that gadgets must be taken from the main executable, not from libraries.
    Gadgets and functions will have fixed offsets from the base address, given by `readelf -e <executable>`.
...
e.g. If we write `%7\$s\x00\x00\x00\x00\x32\x17\x00\x41` into a buffer, when `printf` is called, it will interpret the 7th argument on the stack as a pointer. Then, it will dereference that address and print the string until it hits a null byte. 
...
Relocation read-only (RELRO) marks the .got section (not .got.plt) as read-only after dynamic linking is done. It is enabled with `z relro`.

Bindnow is a feature that forces all dynamic linking to be done eagerly instead of lazily, merging the .got and the .got.plt sections. When combined with RELRO, this ensures that both the .got and .got.plt section is read-only. It is enabled with `-z now`.
