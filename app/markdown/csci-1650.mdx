...
1. [Basic concepts](#basic-concepts) 
    - [ELF](#elf), [C language](#c) (9/10) 
    - [Memory](#memory), [Processes address space](#process-address-space), [Executing a process](#executing-a-process) (9/15) 
...
4. [Shellcode development](#shellcode-development)
    - [Shell code](#shell-code), [Writing shell code](#writing-shell-code) (10/1)
...
NOP sleds are sequences of NOP instructions that slide the execution flow to the shell code. You can put the payload anywhere that fits.
...
Reverse shell code connects back to the attacker's machine and gives the attacker a shell. Anything the attacker types is executed on the victim's machine; and anything the victim's machine outputs is sent back to the attacker.

Bind shell code listens on a port and gives the attacker a shell when they connect to that port.
...
Tricks to reduce shell code size:
...
On the compromised machine, the shell code will:
1. Create a socket
...
2. Add shell code where `mprotect` will return 
