...
The **client** has a domain name (e.g. example.com). It queries the **DNS** to resolve the domain to an IP address. The IP address returned by DNS typically belongs to a traffic entry point, such as a content delivery network (CDN) edge or a load balancer, rather than a single origin server.
...
Requests that reach the origin are handled by a **load balancer**, which distributes traffic across multiple application servers. These servers expose API endpoints using standard HTTP methods (GET, POST, PUT, DELETE) to operate on resources.
...
![Web architecture](/system-design/web-architecture.jpeg)
...
In the **token bucket** design, tokens are added to a bucket at a fixed rate. Each request consumes a token. If the bucket is empty, requests are denied. This allows for bursts of traffic if tokens are available.

![Token bucket](/system-design/token-bucket.png)

In the **leaky bucket** design, requests are added to the bucket and processed at a fixed rate. If the bucket is full, incoming requests are denied. This enforces a constant output rate.

![Leaky bucket](/system-design/leaky-bucket.png)

In the **fixed window counter** design, the number of requests is counted within fixed time windows (e.g., per minute). If the count exceeds the limit, further requests are denied until the next window. This can lead to bursts of traffic at the edges of windows.

![Fixed window counter](/system-design/fixed-window-counter.png)

In the **sliding window log** design, each request is timestamped and stored in a log. To check the rate limit, the log is scanned to count requests within the sliding time window. This provides accurate rate limiting but can be memory-intensive.

![Sliding window log](/system-design/sliding-window-log.png)

In the **sliding window counter** algorithm, the time window is divided into smaller intervals. Each interval maintains a count of requests. To check the rate limit, counts from the relevant intervals are summed. This reduces memory usage compared to the sliding window log.

![Sliding window counter](/system-design/sliding-window-counter.png)

### Rate Limiting Architecture

Rate limiting should be done on the server side to ensure that clients cannot bypass it. Middleware solutions (such as API gateways) can also be used to offload rate limiting from application servers.
...
Within a single server, we can optimize the key value store by:
1. Compressing cold memory 
2. Caching hot memory in an in-memory store in addition to the disk  

Within a distributed key-value store, we need to decide on:
1. Data partitioning
2. Data replication
3. Consistency model & conflict resolution
4. Failure detection and recovery

The **CAP theorem** states that a distributed system can only guarantee two of the following three properties: (1) **consistency**, (2) **availability**, and (3) **partition tolerance**. Because it is not possible to avoid network partitions, distributed KV stores must either be CP or AP.
...
An URL shortener should support the following API endpoints:
1. `POST /api/shorten`
   - Request: `{ "original_url": "https://example.com" }`
   - Response: `{ "short_url": "https://short.ly/abc" }`
2. `GET /\{short_url}`
   - Response: HTTP status code 301 redirect to original URL

To implement URL shortening, there are two options:
- The first option is to hash the original URL to generate a unique key. Most hash functions produce long strings, so take the first $n$ characters of the hash and rehash if a collision is found in the database. To avoid expensive database lookups, a bloom filter can be used to quickly check for potential collisions. 
- The second option is to [generate a unique ID](#unique-id-generator) each time. This ID can also be shorted by converting it to base-62.
...
- [Redis will finally make sense after this video](https://www.youtube.com/watch?v=S7_SOmxO5DA&list=WL&index=38)
