...
[RFocus: Beamforming Using Thousands of Passive Antennas](https://www.usenix.org/conference/nsdi20/presentation/arun)

Problem: A transmitter can direct a more powerful signal beam to a receiver by beamformming with antenna arrays. However, there are physical limits on the number of antennas we can fit on a device.

The idea of RFocus is to increase antenna capacity by putting antennas in the environment. RFocus surface consts of 3200 antennas, all of which are either reflective or transparent. The receiver sends measurements of the received signal strength to the RFocus controller, which then configures the surface.

### Cellular Networks

Cellular networks today are divided into:
1. Radio Access Network (RAN), which consists of radio towers that connect to user equipment (UE)
2. Mobile core, which connects the RAN to the rest of the Internet
